      ...
      <div className="max-w-6xl mx-auto">
        <div className="rounded-full border border-[#1a0f05]/10 bg-[#fffbf0]/95 shadow-[0_8px_30px_rgba(26,15,5,0.12)] backdrop-blur-md">
          <div className="flex items-center justify-between gap-3 px-4 sm:px-6 py-2.5">
            ...
            <div className="hidden md:flex items-center gap-1.5 text-sm font-medium text-[#1a0f05]">
              {NAV_ITEMS.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  onClick={handleNavClick(item.href)}
                  className="rounded-full px-3 py-2 transition-colors hover:bg-[#f97316]/10 hover:text-[#f97316]"
                >
                  {item.label}
                </a>
              ))}
            ...
            <a
              ...
              className="hidden md:inline-flex items-center gap-1.5 rounded-full bg-[#1a0f05] px-4 py-2 text-xs font-semibold text-white transition-colors hover:bg-[#2d1a0a]"
            >
              Order on WhatsApp
            </a>
